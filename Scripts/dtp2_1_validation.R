...
zero.clust <- which(is.na(vaxdata.all$total)|vaxdata.all$total<=1|is.na(vaxdata.all$dtp2_1_count))
...
#All_ages
Numvacc.all    <- round(vaxdata.all$dtp2_1_count,0)
weights.all    <- vaxdata.all$total

#Coordinates
coords.all    <- cbind(vaxcov.all$lon,vaxcov.all$lat)
min.dist      <- as.numeric(summary(dist(coords.all))[1]) #Min of distances between cluster locations #0.003378099
...
write.csv(val.out, "val_out_dtp2_1_rand_ratio.csv")


colnames(ppred.out) <- c("prob.obs", "mean", "sd", "pred.low", "0.5quant", "pred.up") 

write.csv(ppred.out, "val_obs_pred_dtp2_1_rand_ratio.csv")
