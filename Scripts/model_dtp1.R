...
min.dist  <- as.numeric(summary(dist(coords))[1]) #Min of distances between cluster locations
...
#This is 5% of the extent of CIV in the north-south direction (i.e. 0.05*(ymax-ymin))
ymax <- max(coords[,2])
ymin <- min(coords[,2])
r0 <- 0.05*(ymax-ymin) #0.2525117
...
#-------------------------------------------------------------------------------
# Fit model
hyper.prec = list(theta = list(prior="pc.prec", param=c(3, 0.01)))  #param=c(3, 0.01)
...
#save model
save(res, file = paste0(filePathData2,"inla_model_dtp1.rda"))
...
write.csv(param.all, paste0(filePathData2,"parameter_output_dtp1.csv"))
...
write.csv(ds, paste0(filePathData2,"obsvpred_dtp1.csv"))

# Define the range for both axes to ensure they have the same scale
range_vals <- range(c(ds$pred.prob, ds$pred.obs))

# Create the plot with equal x and y limits, with swapped axes
plot(ds$pred.obs, ds$pred.prob, xlim = range_vals, ylim = range_vals, 
     xlab = "Observed", ylab = "Predicted", 
     main = "Observed vs Predicted")

# Add a red line from (0,0)
abline(0, 1, col = "red")
...
writeRaster(rr.mean, "inla_vax_dtp1_mean.tif", overwrite=TRUE)
writeRaster(rr.sd,   "inla_vax_dtp1_sd.tif", overwrite=TRUE)
writeRaster(rr.low,  "inla_vax_dtp1_low.tif", overwrite=TRUE)
writeRaster(rr.up,   "inla_vax_dtp1_up.tif", overwrite=TRUE)
writeRaster(rr.med,  "inla_vax_dtp1_median.tif", overwrite=TRUE)
...
write.csv(dist_out, "SP_estimates_dtp1.csv")
...
#The district-level estimates will have the same ordering as in the shapefile if they have the same no of areas
write.csv(dist_out, paste0(filePathData2, "Prefecture_estimates_dtp1.csv"))
...
#The district-level estimates will have the same ordering as in the shapefile if they have the same no of areas
write.csv(dist_out, paste0(filePathData2, "Region_estimates_dtp1.csv"))
...
writeRaster(rr.80, paste0(filePathData2, "inla_vax_dtp1_80perc_thresh.tif"), overwrite=TRUE)
