...
zero.clust <- which(is.na(vaxdata.all$total)|vaxdata.all$total<=1|is.na(vaxdata.all$dtp3_2_count))
...
#All_ages
Numvacc.all    <- round(vaxdata.all$dtp3_2_count,0)
...
write.csv(val.out, "val_out_dtp3_2_rand_ratio.csv")


colnames(ppred.out) <- c("prob.obs", "mean", "sd", "pred.low", "0.5quant", "pred.up") 
write.csv(ppred.out, "val_obs_pred_dtp3_2_rand_ratio.csv")
