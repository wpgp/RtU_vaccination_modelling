...
zero.clust <- which(is.na(vaxdata$total)|vaxdata$total<=1|is.na(vaxdata$dtp2_1_count))
...
#All_ages
Numvacc    <- round(vaxdata$dtp2_1_count,0)
weights    <- vaxdata$total

#Coordinates
coords    <- cbind(vaxcov$lon,vaxcov$lat)
min.dist  <- as.numeric(summary(dist(coords))[1]) #Min of distances between cluster locations #0.003378099
...
#Prediction grid
# n25.p      <- raster(paste0(filePathData1,"cmr_ppp_2018_1km_Aggregated.tif"))
...
#save model
save(res, file = paste0(filePathData2,"inla_model_dtp2_1.rda"))
...
write.csv(param.all, paste0(filePathData2,"parameter_output_dtp2_1.csv"))
...
write.csv(ds, paste0(filePathData2,"obsvpred_dtp2_1.csv"))
...
save(inv.linpred, file = paste0(filePathData2,"simprob_dtp2_1.rda"))
...
writeRaster(rr.mean, "inla_vax_dtp2_1_mean.tif", overwrite=TRUE)
writeRaster(rr.sd,   "inla_vax_dtp2_1_sd.tif", overwrite=TRUE)
writeRaster(rr.low,  "inla_vax_dtp2_1_low.tif", overwrite=TRUE)
writeRaster(rr.up,   "inla_vax_dtp2_1_up.tif", overwrite=TRUE)
writeRaster(rr.med,  "inla_vax_dtp2_1_median.tif", overwrite=TRUE)
...
write.csv(dist_out, "SP_estimates_dtp2_1.csv")
...
#The district-level estimates will have the same ordering as in the shapefile if they have the same no of areas
write.csv(dist_out, paste0(filePathData2, "Prefecture_estimates_dtp2_1.csv"))
...
#The district-level estimates will have the same ordering as in the shapefile if they have the same no of areas
write.csv(dist_out, paste0(filePathData2, "Region_estimates_dtp2_1.csv"))
...
writeRaster(rr.80, paste0(filePathData2, "inla_vax_dtp2_1_80perc_thresh.tif"), overwrite=TRUE)
